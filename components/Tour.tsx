...
const Tour = () => {
  ...
  const handleNavigation = (type: string, tourId: string) => {
    setIsLoading(tourId); 
    setTimeout(() => {
      router.push(`/tour/${type}/${tourId}`); 
    }, 500); 
  };
      ...
      >
        {tourList.map((tour) => (
          <SwiperSlide key={tour.id} className="w-full max-w-[300px] sm:max-w-[400px] mr-2.5 sm:mr-5">
            <div
              onClick={() => handleNavigation(type, tour.id)}
              className="relative bg-gray-800/50 rounded-2xl overflow-hidden shadow-lg transition-all duration-300 hover:shadow-2xl hover:scale-100 cursor-pointer"
            >
              ...
              <div className="p-2 sm:p-3 text-center transition-colors duration-300 group-hover:text-white">
                ...
                <button
                  onClick={(e) => {
                    e.stopPropagation(); 
                    handleNavigation(type, tour.id); 
                  }}
                  disabled={isLoading === tour.id}
                  className={`w-[90%] text-gray-100 px-2 sm:px-4 py-2 sm:py-3 rounded-lg text-base sm:text-base font-medium transition-all duration-300 ${
                    isLoading === tour.id
                      ? "bg-gray-600 cursor-not-allowed"
                      : "bg-green-600 hover:bg-gradient-to-r hover:from-teal-600 hover:to-blue-700 hover:scale-105"
                  }`}
                >
                  {isLoading === tour.id ? (
