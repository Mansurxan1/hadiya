...
const SuccessModalContent = ({ onClose }: { onClose: () => void }) => {
  ...
  useEffect(() => {
    ...
    const sendNotification = async () => {
      try {
        ...
        if (paymentStatus === "success" && orderDetails) {
          ...
          }\n🆔 ${t("transaction_id")}: ${orderDetails.transactionId}`;
        } else if (paymentStatus === "failed" && orderDetails) {
          message = `${t("payment_failed")}\n🆔 ${t("order_id")}: ${
            orderDetails.orderId
          }\n${t("error_code")}: ${orderDetails.errorCode}`;
        }
    ...
    if (paymentStatus !== "unknown" && orderDetails) {
      sendNotification();
    }
        ...
        {orderDetails && (
          <div className="bg-gray-50 p-4 rounded-xl mb-6 text-left shadow-inner border border-gray-100">
            ...
            {orderDetails.errorCode && (
              <p className="text-sm text-gray-600">
                <strong>{t("error_code")}:</strong> {orderDetails.errorCode}
              </p>
            )}
            <p className="text-sm text-gray-600">
              <strong>{t("date")}:</strong> {orderDetails.date}
            </p>
          </div>
        )}
