...
const TourDetail = () => {
  ...
  const validateForm = () => {
    ...
    if (!phoneDigits) {
      ...
      newErrors.phone =
        t("errors.phoneInvalid") || "Telefon raqami to'liq emas";
    }
    ...
    if (validateForm()) {
      ...
      try {
        ...
        if (data.success && data.redirectUrl) {
          console.log(
            `Переход к оплате тура ${tour.id} на сумму ${tour.price}`
          );
      ...
      <div className="max-w-[1700px] mx-auto px-5 py-10">
        ...
        >
          <button
            ...
            className="bg-gradient-to-br bg-sky-500/70 text-white px-6 py-3 rounded-lg font-medium text-lg shadow-md hover:shadow-lg transition-all duration-300 hover:from-sky-600 hover:to-sky-700 hover:scale-105"
          >
            {t("back")}
          </button>
        </motion.div>
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-3 gap-6 mb-6">
          <ScrollAnimatedCard variants={cardVariants}>
            <motion.div
              className="text-white bg-sky-800/70 p-6 rounded-xl shadow-md h-full flex flex-col justify-between"
        ...
        <div className="grid grid-cols-1 gap-6">
          ...
          {type !== "internal" && t(`${translationKey}.${tour.id}.hotel`) && (
            <ScrollAnimatedCard variants={cardVariants}>
              <motion.div
                className="text-white bg-sky-800/70 p-3 rounded-xl shadow-md h-full flex flex-col justify-between"
                whileHover={{ scale: 1.02, transition: { duration: 0.3 } }}
              >
                <div>
                  <h2 className="text-xl md:text-2xl font-semibold">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex px-5 items-center justify-center bg-black bg-opacity-50 z-50">
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            className="bg-sky-700 p-6 rounded-xl shadow-lg max-w-md w-full"
          >
